 ...
 * Adjusted by Karel Reinhard to work with banana pi/raspberry pi
 */
...
int NewRemoteReceiver::_interrupt;
...
void showCode(NewRemoteCode receivedCode) {
    std::cout << "Received something "  << receivedCode.address <<" unit " << receivedCode.unit <<" period " << receivedCode.period << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    //Set prio low to use less cycles
    ...
    //wiringPiISR(1 ,CHANGE,&lala);
    std::cout << "started!" << std::endl;
    NewRemoteReceiver::init(1, 1, &showCode);
    while(1){
        usleep(60000);
    }
...
void detachInterrupt(int _interrupt){
    std::ostringstream  ostr;
    ostr << "/usr/local/bin/gpio edge "  << _interrupt << " none";
    std::string sstr(ostr.str());
    system (sstr.c_str());
    std::cout << "detatched!" << std::endl;
}

void NewRemoteReceiver::init(int interrupt, unsigned short minRepeats, NewRemoteReceiverCallBack callback) {
